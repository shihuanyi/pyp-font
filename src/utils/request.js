...
const service = axios.create({
    ...
    headers: {
        ...
        'Case': process.env.VUE_APP_IS_CASE,
        'model': 'default'
    }
})
    ...
    response => {
        ...
        if (res) {
            ...
            if (res.code === 0) { // code 为0请求正常
                ...
                if (res.code === 700010) {
                    store.dispatch('user/refreshLink', window.location.origin + window.location.pathname)
                        .then(() => {
                            window.location.href = store.state.user.link
                        })
                }
                if (res.code === 700009) {
